...
def scoring(df,num_downsample = 5000):
    if len(df) > num_downsample: # CASE 1 : Dataset very big 
        tmp = [] 
        for i in range(3):
            np.random.seed(i)
            random.seed(i)
        ...
        scores = list(np.median(np.stack(tmp),axis=0)) # 3 different samples, take the median to have more precise estimation of scores.
    else: # If small dataset, compute directly the scores
        df = preprocessing(df)
        sc1 = features_dpt_entropy(df, num_bins = 10)
        sc2 = features_homology_dpt_entropy(df,num_bins = 3)
        sc3 = features_vector(df)
